	...
	switch( $_POST['submit_del'] )
	{
		...
		case 1:
			for( $i=0; $i<count($_POST['del']); $i++ )
			{
				...
				$nachricht->delMessage("empfaenger");	
			}
			break;		
		//ALle Einträge löschen
		case 2:	
			//Alle NAchrichten, die dem  User gehören durchlaufen!
			$db->query("SELECT ID_Nachricht FROM t_nachrichten WHERE ID_User = ".$_SESSION['user']->getUserID()."");
		...
		case 3:
			//Alle NAchrichten, die dem  User gehören durchlaufen!
			$db->query("SELECT ID_Nachricht FROM t_nachrichten WHERE ID_User = ".$_SESSION['user']->getUserID()." AND Status = 'gelesen'");
			while( $row = $db->fetch_array() )
			{
				//nachrichtenobjekt erzeugen
				$nachricht = new NACHRICHT();
...
if( $daten_nachrichten['INBOX_GESAMT'] == 0 )	//Keine Nachrichten vorhanden
{
...
{	    
	...
	$daten_column['SPALTE'] = "Absender:";
	$tpl_detail->setObject("nachrichten_detail_topic", $daten_column);
	
	//Lade alle Ereignisse
    $db->query("SELECT ID_Nachricht FROM t_nachrichten WHERE ID_User = ".$_SESSION['user']->getUserID()." AND Deleted != 'empfaenger' ORDER BY Datum DESC");
    ...
    while( $row = $db->fetch_array() )
    {
        ...
        $daten_detail['ABSENDER']	= $msg->getAbsender();
        $daten_detail['BETREFF'] 	= "<a href=\"#\" onClick=\"PopUp('nachrichten_show.php?action=inbox&ID=".$msg->getID()."', 550, 550)\">".$msg->getBetreff()."</a>";
