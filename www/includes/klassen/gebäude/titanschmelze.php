...
extends GEBÄUDE
{
	...
	//Deklarationen
	var $Faktor = 2;				//Umrechnugnsfaktor... wie viel Eisen wird für eine Einheit Stahl benötigt?
	var $ID = 13;					//Welche ID besitzt das titanschmelzegebäude? (für interne Berechnugnszwecke!)
	var $Auslastung;				//Wie hoch ist die Auslastung in % der titanschmelze
	var $user;						//Zeiger auf user-objekt
	var $ID_Kolonie;				//KOlonieID des Gebäudes
	var $ID_Rohstoff = 7;			//Die ID des Wassers - Rohstoffes wird vverbraucht
	var $ID_Rohstoff_Gewinn = 8;	//Stahl wird produziert
    ...
    function TITANSCHMELZE(&$db, &$user, $ID_Kolonie, $ENERGIE)
    {
    	//Debugmeldung
    	$this->debug("Erzeuge TITANSCHMELZE - Objekt");
		...
		//Auslastung der titanschmelze laden
		...
		//Debugmeldung
    	$this->debug("Titanschmelze des Users <i>".$this->user->getUserID()."</i> wird auf ".($this->Auslastung * 100)."% gefahren");
    }   	
